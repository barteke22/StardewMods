...
namespace StardewMods
{
    ...
    public class ModEntry : Mod
    {
        ...
        private readonly PerScreen<List<int>> oldGeneric = new PerScreen<List<int>>();
        private readonly PerScreen<Dictionary<int, int>> fishFailed = new PerScreen<Dictionary<int, int>>();
        ...
        private void RenderedHud(object sender, RenderedHudEventArgs e)
        {
            ...
            if (iconMode != 3)
            {
                ...
                if (foundWater)
                {
                    ...
                    //{
                    //    fishHere.Value.Add(100 + i);
                    //    fishChances.Value.Add(100 + i, 1000);
                    //}

                    foreach (var fish in fishHere.Value)
                    {
                        int percent = (int)Math.Round((float)fishChancesSlow.Value[fish] / (float)fishChancesSlow.Value[-1] * 100); //chance of this fish

                        if (iconCount < maxIcons && fishChancesSlow.Value.ContainsKey(fish) && percent > 0)
                        {
        ...
        private void AddGenericFishToList(string locationName, int fishingLocation)         //From GameLocation.cs getFish()
        {
            ...
            if ((tempFish.Count == 0 && oldGeneric.Value.Count != 0) || tempFish.Count > 0 && (!(new HashSet<int>(oldGeneric.Value).SetEquals(tempFish))))//reset lists if generic list changed
            {
                oldGeneric.Value = tempFish.ToList();
                fishFailed.Value = new Dictionary<int, int>();
                fishHere.Value = new List<int> { 168 };
                fishChances.Value = new Dictionary<int, int> { { -1, 0 }, { 168, 0 } };
                fishChancesSlow.Value = new Dictionary<int, int>();
                ...
                foreach (var key in oldGeneric.Value)
                {
        ...
        private void AddFishToListDynamic()                                                  //very performance intensive check for fish fish available in this area - simulates fishing
        {
            ...
            {
                int freq = (isMinigame.Value) ? 5 : extraCheckFrequency / totalPlayersOnThisPC; //minigame lowers frequency
                for (int i = 0; i < freq; i++)
                {
                    ...
                    if (fishChances.Value[-1] < int.MaxValue) //percentages, slow version (the one shown) is updated less over time
                    {
                        if (f >= 167 && f <= 172) fishChances.Value[168]++;
                        else if (!fishHere.Value.Contains(f))
                        {
                            fishChances.Value = new Dictionary<int, int> { { -1, 0 } };//reset % on new fish added
                            foreach (var fish in fishHere.Value) fishChances.Value.Add(fish, 1);
                            fishChancesSlow.Value = new Dictionary<int, int>();
                            fishChancesModulo.Value = 1;
                    ...
                    {   
                        fishFailed.Value[f] = 2000;
                        foreach (var key in fishFailed.Value.Keys.ToList())
                        {
                            if (fishFailed.Value[key] != f) fishFailed.Value[key]--;
                            if (fishFailed.Value[key] < 1) oldGeneric.Value = null;
                        } 
                    }
                }
            }
        ...
        private void OnModMessageReceived(object sender, ModMessageReceivedEventArgs e)
        {
            ...
            {
                //if (e.Type == "fishCaught" && Game1.player.UniqueMultiplayerID == e.ReadAs<long>()) oldTime.Value = -1;
                if (e.Type == "whichFish") miniFish.Value = e.ReadAs<int>();
            }
        }
    }
}
